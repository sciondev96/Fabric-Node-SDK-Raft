        ...
        if (fcn == "readMarble") {
            if (response_payloads) {
                for (let i = 0; i < response_payloads.length; i++) {
                    res = JSON.parse(response_payloads[i]);
                    logger.info(args[0] + ' now has ' + res.size +
                        ' marbles');
                }
                // let res = JSON.parse(response_payloads[0]);
                return args[0] + ' now has ' + res.size.toString(10) + ' marbles';
            } else {
